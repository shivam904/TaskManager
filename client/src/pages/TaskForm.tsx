...
import { createTask, fetchTaskById, updateTask, clearTaskError } from '../store/slices/taskSlice';
...
const TaskForm: React.FC = () => {
  ...
  useEffect(() => {
    if (id && currentTask) {
      ...
      setValue('dueDate', new Date(currentTask.dueDate).toISOString().split('T')[0]);
      setValue('assignedTo', typeof currentTask.assignedTo === 'object' 
        ? currentTask.assignedTo._id 
        : currentTask.assignedTo);
    }
  }, [id, currentTask, setValue]);
  ...
  const onSubmit = async (data: TaskFormData) => {
    try {
      ...
      // Dispatch action based on whether we're creating or editing
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          {id ? 'Update the task details below.' : 'Fill in the details to create a new task.'}
        </p>
      </div>
